...
export class StudyTrackerComponent {
  ...
  private async loadMonth() {
    ...
    this.trackerService.trackerMonthData$(user.uid, 'study', monthId).subscribe((doc: any) => {
      const w = doc?.days;
  ...
  async saveMonth() {
    try {
      ...
      if (!user) return;
      const monthId = this.trackerService.getMonthId();
      await this.trackerService.saveTrackerMonthCollection(user.uid, 'study', monthId, { days: this.days });
      this.saved = true;
      this.edit = false;
  ...
  async deleteDay(dayIdx: number) {
    ...
    if (!user) return;
    const monthId = this.trackerService.getMonthId();
    await this.trackerService.deleteTrackerDayMonthCollection(user.uid, 'study', monthId, dayIdx);
