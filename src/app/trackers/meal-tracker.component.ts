...
export class MealTrackerComponent {
  ...
  private async loadMonth() {
    ...
    this.trackerService.trackerMonthData$(user.uid, 'meals', monthId).subscribe((doc: any) => {
      const w = doc?.days;
  ...
  async saveMonth() {
    try {
      ...
      if (!user) return;
      const monthId = this.trackerService.getMonthId();
      await this.trackerService.saveTrackerMonthCollection(user.uid, 'meals', monthId, { days: this.days });
      this.saved = true;
      this.edit = false;
  ...
  async deleteDay(dayIdx: number) {
    ...
    if (!user) return;
    const monthId = this.trackerService.getMonthId();
    await this.trackerService.deleteTrackerDayMonthCollection(user.uid, 'meals', monthId, dayIdx);
