...
export class ScreenTimeTrackerComponent {
  ...
  private async loadWeek() {
    ...
    this.trackerService.trackerWeekData$(user.uid, 'screentime', weekId).subscribe((doc: any) => {
      const w = doc?.days;
  ...
  async saveWeek() {
    try {
      ...
      if (!user) return;
      const weekId = this.trackerService.getWeekId();
      await this.trackerService.saveTrackerWeekCollection(user.uid, 'screentime', weekId, { days: this.days });
      this.saved = true;
      this.edit = false;
  ...
  async deleteDay(dayIdx: number) {
    ...
    if (!user) return;
    const weekId = this.trackerService.getWeekId();
    await this.trackerService.deleteTrackerDayCollection(user.uid, 'screentime', weekId, dayIdx);
